...
export default function MainPage() {
  ...
  const { data: listOfCharacters, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { results } = await getApi<CharacterResponse>(
  ...
  const filteredList = listOfCharacters?.filter((character) =>
    character[selectedFilter as keyof Character]
      ?.toLowerCase()
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-5 gap-4">
          {isLoading ? (
            ...
            <NoResults
              ...
              message="No characters matched your search. Please try again."
            />
          )}
        </div>
        <div>
